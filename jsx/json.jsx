...
if (!$.global.hasOwnProperty("JSON")) {
	...
	// `allowUTF16` : Reserved. (For the time being, output is always a full ASCII string.)
	{
		...
		if (quote == q.unsafeQuote) {
			...
			q.unsafeQuote ||
				(r = r.replace(RegExp["_" + o[altQuote]], altQuote));
		}
	...
	// [REM] The once-call function parses ExtendScript's native toSource behavior.
	{
		...
		if (a.length != o.length) {
			err =
				"A fatal error occured while analyzing String.prototype.toSource.";
		}
	...
	$.global.JSON["~"].setup({
		...
		// => 1 (broken) | 0 (not-broken)
		{
			return +(
				...
				(
					callee.Q || (callee.Q = /@(find|change).+shadow-settings$/)
				).test(s)
			);
		},
		...
		{
			if ((q = callee.Q).hasOwnProperty((k = o.toSpecifier())))
				return q[k];
